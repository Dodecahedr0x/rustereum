//! Procedural macro backing rustereum's `#[contract]` DSL.
//!
//! This crate provides a single [`macro@contract`] attribute macro. rustereum
//! is an experimental Rust DSL that compiles EVM smart contracts written as
//! ordinary Rust `struct`s and `impl` blocks down to Solidity, then to bytecode
//! via `solc`. `#[contract]` is the front end of that pipeline: it reads the
//! annotated items and lowers them into a small intermediate representation
//! (IR), which the `rustereum` runtime crate turns into Solidity source.
//!
//! This crate has no dependency on the `rustereum` runtime crate, so the paths
//! it emits (`::rustereum::ir::…`, `::rustereum::vm::…`, `::rustereum::driver::…`)
//! resolve in the *caller's* crate at expansion time. For that reason, runtime
//! types are referenced here as plain code text (e.g. `rustereum::ir::Method`)
//! rather than as intra-doc links.
//!
//! # The DSL surface
//!
//! `#[contract]` is applied to two kinds of items:
//!
//! ## A storage struct
//!
//! A `struct` with named fields declares contract storage. In v1 every field
//! must be `u256`:
//!
//! ```ignore
...
//!
//! An inherent `impl` declares the contract's methods, constructor, and
//! base-constructor wiring. Method bodies are a deliberately tiny expression
//! language (see below):
//!
//! ```ignore
//! #[contract]
...
//! ```
//!
//! - A `&self` receiver marks a **view** method; `&mut self` marks a
//!   **state-changing** method.
//! - Parameters and return values may be `u256`, `Address`, or `bool`.
//!
//! ## A trait `impl` for inheritance
//!
//! A trait `impl` with an empty body — `impl Parent for Contract {}` — declares
//! that `Contract` inherits the Solidity contract behind the binding trait
//! `Parent` (a trait generated by `rustereum add`, carrying `SOL_NAME` and
//! `SOL_IMPORT` consts). v1 supports exactly one parent per contract.
//!
//! ```ignore
...
//! ```
//!
//! ## Constructor and modifier attributes
//!
//! Inside the inherent `impl`, two helper attributes control constructor and
//! modifier generation:
//!
//! - `#[constructor(Parent(args...))]` marks a method as the constructor and
//!   forwards `args` to the base constructor. Each arg is either an **identifier**
//!   (referencing one of this constructor's own parameters — camelCased to match
//!   the Solidity param name) or a **string/int/bool literal** (emitted verbatim,
//!   e.g. `ERC20("MyToken", "MTK")`). The constructor method is excluded from the
//!   emitted method table.
//! - `#[modifier(snake_case)]` applies an inherited Solidity modifier to a
//!   method; the name is camelCased (`only_owner` → `onlyOwner`).
//!
//! ```ignore
//! #[contract]
//! impl Counter {
...
//!
//! Method and constructor bodies accept only:
//!
//! - `self.<field>` — a storage load or assignment target,
//! - integer literals,
//! - `+` (binary addition), `+=` (add-assign to storage), `=` (assign to
//!   storage),
//! - a trailing tail expression or `return <expr>` as the returned value.
//!
//! Anything else produces a `compile_error!` diagnostic pointing at the
//! offending span.
//!
//! # What the macro generates
//!
//! For a **struct**, it re-emits the struct (so the Rust type still exists) plus
//! a `rustereum::ir::ContractStorage` impl exposing the contract name and its
//! `rustereum::ir::Field` list.
//!
//! For a **trait impl**, it re-emits the impl plus a
//! `rustereum::ir::ContractInherits` impl whose single `rustereum::ir::Parent`
//! reads `SOL_NAME`/`SOL_IMPORT` off the binding trait.
//!
//! For an **inherent impl**, it emits three things:
//!
//! 1. A re-emission of the original `impl` with the `#[constructor]`/`#[modifier]`
//!    helper attributes stripped, so the bodies still type-check as native Rust
//!    against `u256`/`Address`/`bool`.
//! 2. A `rustereum::ir::ContractMethods` impl carrying the lowered
//!    `rustereum::ir::Method` list, the optional `rustereum::ir::Constructor`,
//!    and the base-initializer list.
//! 3. A **typed test client** so tests drive the deployed contract without
//!    hand-writing ABI encoding:
//!    - `Counter::compile()` / `Counter::compile_with(&opts)` — assemble the IR
//!      and compile it to a `rustereum::driver::Artifact` (bytecode + ABI). The
//!      default project root is the contract crate's `CARGO_MANIFEST_DIR`.
//!    - `Counter::deploy(&mut vm, &artifact, ctor_args…)` — deploy and return a
//!      `CounterClient`.
//!    - `CounterClient` — one method per contract method. View methods take just
//!      the VM and return the decoded value (panicking on revert); mutating
//!      methods take an explicit `caller` and return `Result<Ret, Revert>`.
//!
//! The generated client references only `::rustereum::vm` and
//! `::rustereum::driver`, so it compiles without the runtime's optional
//! `testing` (revm) feature.
//!
//! # Naming
//!
//! Identifiers are written `snake_case` in Rust and converted to `camelCase` in
//! the emitted Solidity (and therefore in the ABI selectors the client builds),
//! so `add_ten` in Rust becomes `addTen` on-chain.
...
/// The `#[contract]` attribute macro: the front end of the rustereum
/// compilation pipeline.
///
/// Apply it to a storage `struct`, an inherent `impl`, or a trait `impl`. What
/// it accepts and generates for each is described in detail at the
/// [crate level](crate). In brief:
///
/// - **`struct`** (named `u256` fields) → re-emitted struct + a
///   `rustereum::ir::ContractStorage` impl.
/// - **inherent `impl`** → re-emitted impl (helper attrs stripped) +
///   `rustereum::ir::ContractMethods` impl + a typed test client
///   (`compile`/`compile_with`/`deploy` on the type, plus a `<Name>Client`).
/// - **trait `impl`** (`impl Parent for C {}`) → re-emitted impl + a
///   `rustereum::ir::ContractInherits` impl declaring the single parent.
///
/// The `_attr` token stream is ignored: `#[contract]` takes no arguments.
///
/// # Supported subset (v1)
///
/// - Field / param / return types: `u256`, `Address`, `bool`.
/// - Receivers: `&self` (view) and `&mut self` (state-changing). Associated
///   functions (no receiver) are not supported.
/// - Body expressions: `self.<field>`, integer literals, `+`, `+=`, `=`, and a
///   trailing tail expression or `return`.
/// - Helper attributes on methods: `#[constructor(Parent(args...))]` and
///   `#[modifier(snake_case)]`.
///
/// # Diagnostics
///
/// Unsupported syntax is reported as a `compile_error!` at the offending span
/// while the original item is re-emitted where possible (so downstream
/// "no method named…" errors don't cascade). Examples:
///
/// - applying `#[contract]` to anything but a struct or impl,
/// - a struct with unnamed fields or a non-`u256` field,
/// - a non-`u256`/`Address`/`bool` parameter or return type,
/// - a non-identifier parameter pattern,
/// - a method without a `self` receiver,
/// - a body statement outside the supported expression subset.
///
/// # Example
///
/// ```ignore
